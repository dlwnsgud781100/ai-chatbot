...
export class PlayerController {
  constructor({events,player,renderer,world,combat,interaction,animator,gameState}) {this.events=events;this.player=player;this.renderer=renderer;this.world=world;this.combat=combat;this.interaction=interaction;this.animator=animator;this.gameState=gameState;this.keys=new Set();this.velocityY=0;this.verticalOffset=0;this.dodgeTime=0;this.dashTime=0;this.knockbackTime=0;this.actionLockTime=0;this.dodgeDirection=new THREE.Vector3();this.knockbackVelocity=new THREE.Vector3();this.lastNetworkIntent=0;this._onKeyDown=this.onKeyDown.bind(this);this._onKeyUp=this.onKeyUp.bind(this);this._onMouseDown=this.onMouseDown.bind(this);}
  bind(){window.addEventListener('keydown',this._onKeyDown);window.addEventListener('keyup',this._onKeyUp);window.addEventListener('mousedown',this._onMouseDown);window.addEventListener('pointerdown',()=>this.combat.feedback.unlock(),{once:true});}
  ...
  onMouseDown(event){if(event.button===0&&this.gameState.is(GAME_STATE.PLAYING)&&!event.target.closest('button'))this.combat.useAction(this.isAirborne()?'air':'light');}
  jump(){if(this.verticalOffset>.02||this.actionLockTime>.1)return;this.velocityY=CONFIG.player.jumpVelocity;this.player.state='jump';}
  ...
  update(delta,now){if(!this.gameState.is(GAME_STATE.PLAYING))return;const position=this.renderer.player.position,movement=this.getMovementVector();let movingSpeed=0;this.actionLockTime=Math.max(0,this.actionLockTime-delta);
    ...
    this.velocityY-=CONFIG.player.gravity*delta;this.verticalOffset+=this.velocityY*delta;if(this.verticalOffset<=0){this.verticalOffset=0;this.velocityY=0;}position.y=this.world.getGroundHeight(position.x,position.z)+this.verticalOffset;this.world.constrainPosition(position);this.player.position.x=position.x;this.player.position.y=position.y;this.player.position.z=position.z;this.animator?.update(delta,movingSpeed);this.interaction.update(position);this.events.emit(EVENT.PLAYER_CHANGED,{player:this.player,velocity:movingSpeed});
