...
export class GameBootstrap {
  ...
  start(state){document.querySelector('#title-screen').classList.add('hidden');document.querySelector('#game-canvas').focus?.();state.set(GAME_STATE.PLAYING,'field-start');this.services.get('events').emit(EVENT.NOTIFY,{message:'변경 탐사가 시작되었습니다. 감시자 아린에게 말을 걸어 보세요.',type:'info'});}
  openDialogue(dialogue,state){this.dialogue=dialogue;document.querySelector('#dialogue-speaker').textContent=dialogue.speaker;document.querySelector('#dialogue-text').textContent=dialogue.text;const choices=document.querySelector('#dialogue-choices');choices.innerHTML='';for(const choice of dialogue.choices??[]){const button=document.createElement('button');button.type='button';button.textContent=choice.label;button.onclick=()=>{const action=choice.action;this.closeDialogue(state);action?.();};choices.append(button);}document.querySelector('#dialogue-box').classList.remove('hidden');state.set(GAME_STATE.DIALOGUE,'interaction');this.dialogueReadyAt=Date.now()+160;}
