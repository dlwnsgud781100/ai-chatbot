...
export class DamageSystem {
  ...
  resolvePlayerHit(action,playerStats,target,{bonusMultiplier=1}={}) { return this.calculate({base:action.baseDamage,attack:playerStats.attack,defense:target.data.defense,type:action.type,resistances:target.resistances??target.data.resistances,criticalChance:action.id==='ultimate' ? .3 : .12,criticalMultiplier:action.id==='heavy'?1.75:1.55,bonusMultiplier}); }
